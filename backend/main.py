...
PDF_UPLOAD_DIR = "backend/static/pdfs"
...
async def upload_pdf(file: UploadFile = File(...)):
    ...
    file_path = f"{PDF_UPLOAD_DIR}/{unique_filename}"
    ...
    return {
        "url": f"http://localhost:8000/static/pdfs/{unique_filename}",
        "file_size": size_str
    }
...
def read_test(test_id: int, admin: bool = False, db: Session = Depends(get_db)):
    ...
    return {
        **test.__dict__,
        "questions": questions,
        "total_questions_in_bank": len(all_questions)  # Include this for admin info
    }
